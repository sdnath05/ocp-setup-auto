...
resource "proxmox_vm_qemu" "ocp-vms-create" {
    count               = length(var.ocp_machines2)

    vmid                = var.ocp_machines2[count.index].vmid
    name                = var.ocp_machines2[count.index].vmname
    target_node         = var.ocp_machines2[count.index].nodename
    memory              = var.ocp_machines2[count.index].memory
    balloon             = var.ocp_machines2[count.index].memory
    ...
    # os_type             = "cloud-init"
    # clone               = var.ocp_machines2[count.index].template # The name of the template
    # args                = var.ocp_machines2[count.index].args

    # Cloud-Init configuration
    # cicustom            = var.ocp_machines2[count.index].cicustom
    # ipconfig0           = "ip=dhcp,ip6=dhcp"
    # skip_ipv6           = true
    # ciuser              = var.ocp_machines2[count.index].user
    # cipassword          = var.ocp_machines2[count.index].password
    ...
    cpu {
        cores           = var.ocp_machines2[count.index].numcpus
        type            = "host"
    }
    ...
    #                 discard         = true
    #                 storage         = var.ocp_machines2[count.index].stoageGroup
    #                 size            = var.ocp_machines2[count.index].size
    #                 backup          = true
    #             }
    #         }
    #     }
        ...
        ide {
            ide1 {
                cdrom {
                    ...
                    iso = "local:iso/${var.ocp_machines2[count.index].iso_file}"
                }
            }
        }
    ...
    network {
        ...
        link_down       = false
        model           = "virtio"
        macaddr         = var.ocp_machines2[count.index].macaddr
        tag             = 50
    }
}
